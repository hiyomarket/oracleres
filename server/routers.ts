  ...
  lottery: router({
    ...
    generate: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 天氣加成：依天氣狀況調整能量分數
        if (input?.weather?.weatherElement) {
          const weatherBoost: Record<string, number> = {
            fire: result.energyAnalysis.todayElement === 'fire' ? 0.8 : 0.2,
            water: result.energyAnalysis.todayElement === 'water' ? 0.8 : 0.2,
            wood: result.energyAnalysis.todayElement === 'wood' ? 0.8 : 0.2,
            metal: result.energyAnalysis.todayElement === 'metal' ? 0.8 : 0.2,
            earth: result.energyAnalysis.todayElement === 'earth' ? 0.8 : 0.2,
          };
          const boost = weatherBoost[input.weather.weatherElement] ?? 0;
          result.energyAnalysis.overallLuck = Math.min(10, result.energyAnalysis.overallLuck + boost);
          result.energyAnalysis.recommendation += `。天氣狀況（${input.weather.condition ?? ''}，${input.weather.temperature ?? ''}°C）與今日${result.energyAnalysis.todayElement === input.weather.weatherElement ? '五行共振，能量加成' : '五行互补，穩中有變'}`;
        }
    ...
    scratchStrategies: publicProcedure
      ...
      .query(({ input }) => {
        ...
        return generateScratchStrategies(date);
      }),
  ...
  calendar: router({
    ...
    monthly: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const { getMoonPhase } = await import('./lib/moonPhase');
        const {
          solarToLunar, getDayHourFortunes, getDayAuspicious,
