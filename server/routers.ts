    ...
    topicAdvice: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (db) {
          ...
          const configMap = Object.fromEntries(configs.map((c: any) => [c.configKey, c.configValue]));
          const pointsCost = (configMap["divination_cost"] as number) ?? 30;
          ...
          if (balance < pointsCost) {
            throw new TRPCError({ code: 'PRECONDITION_FAILED', message: `積分不足，問卜需要 ${pointsCost} 點積分` });
          }
          const topicLabels: Record<string, string> = { work: '工作', love: '愛情', health: '健康', wealth: '財運', decision: '決策' };
          await db.update(users).set({ pointsBalance: balance - pointsCost }).where(eq(users.id, ctx.user.id));
