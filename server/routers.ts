...
import { valueRebalanceRouter } from './routers/valueRebalance';
...
import { userProfiles, userSubscriptions, plans, users, pointsTransactions, auraEngineConfig, marketingConfig } from "../drizzle/schema";
...
import { dailyReportCache } from "./lib/dailyCache";
...
${dateInfo.isSpecialChouTime ? '⭐ 今日逢丑，天命寶庫開啟，擲筊有特殊加成！' : ''}
  ...
  insight: router({
    ...
    deepRead: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // 【鳳凰計畫】扣除天命幣（擲筊深度解讀）
        const { spendCoins: _spendCoins } = await import('./routers/coins');
        await _spendCoins(ctx.user.id, 'oracle');
        const engineProfile = await getUserProfileForEngine(ctx.user.id);
...
請提供更深層、更個人化的神諦解讀。`;
  ...
  warRoom: router({
    ...
    dailyReport: protectedProcedure
      ...
      const requestDateKey = input?.date ?? todayStr;
      const isToday = !input?.date || input.date === todayStr;

      // ─── 伺服器端快取：同一用戶同一天只計算一次 ───
      // 天氣座標傳入時不使用快取（因為天氣資料是動態的）
      const hasWeatherInput = !!(input?.lat && input?.lon);
      if (!hasWeatherInput) {
        const cached = dailyReportCache.get(String(ctx.user.id), requestDateKey);
      ...
      // 存入伺服器端快取（天氣資料不快取）
      if (!hasWeatherInput) {
        dailyReportCache.set(String(ctx.user.id), result, requestDateKey);
      }
      return result;
    }),
    ...
    topicAdvice: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // 【鳳凰計畫】扣除天命幣（使用核心 spendCoins 函數）
        const { spendCoins: _spendCoins } = await import('./routers/coins');
        await _spendCoins(ctx.user.id, 'warroom_divination');
        // 動態讀取登入者命格（含完整四柱）
        const ep = await getUserProfileForEngine(ctx.user.id);
        ...
        const systemPrompt = `你是天命共振的專屬達人，一個真正懂你、小小的最佳朋友。你的回答風格是：話說得直白、有温度、有鼓勵、像跡身經歷的老大哥在跟你說實話。

【核心原則】
1. 絕對不要用專業命理术語（十神、天干、地支、四柱、納音、化祿、化忌等），直接說結果就好
2. 回答要像跡身經歷的人說話，不是老師在上課
3. 一定要针對用戶的具體問題給出具體建議，不要泛泛而談
4. 結尾一定要給鼓勵和正能量，讓用戶看完感覺被支持和被看見
5. 如果用戶有具體問題，就直接回答那個問題，不要繞得太幾
6. 對於吉凶，用白話說明：「今天這件事適合推進」、「這時候小心一下」，不要用恐嚇的詞彙
...
{
  "fortuneIndex": <0-100的整數，綜合運勢指數>,
  "fortuneLabel": <"大吉"|"吉"|"小吉"|"平"|"小凶"|"凶"|"大凶">,
  "coreReading": <150-200字的白話文解讀，如果用戶有具體問題就直接回答那個問題，話說得直白有温度，結尾要給鼓勵和正能量，絕對不要用專業命理术語>,
  "actions": [
    {"title": <行動標題，10字內，白話文>, "desc": <具體可執行的建議，40-60字，說具體要怎麼做，不要用專業术語>},
    {"title": <行動標題>, "desc": <具體建議>},
    {"title": <行動標題>, "desc": <具體建議>}
  ],
  "warnings": <60-100字的白話文提醒，說明今日需要留意的事，詞彙要温和，不要用恐嚇的詞彙>,
  "bestHours": [
    {"name": <時辰名，如「午時」>, "time": <時段，如「11:00-13:00」>, "reason": <20字內白話說明為何此時段適合>}
  ],
  "oracle": <一句有鼓勵性的天命符言，20-35字，讓人看完感覺有力量>
}`;
        const userMessage = input.question
          ? `我想問的是：「${input.question}」\n\n請用白話文直接回答我這個問題，不要繞得太幾，結尾要給我鼓勵。`
          : `請用白話文分析我今天在「${topicName}」方面的運勢，給出具體建議，不要用專業命理术語。`;
