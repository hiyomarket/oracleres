...
${dateInfo.isSpecialChouTime ? '⭐ 今日逢丑，天命寶庫開啟，擲筊有特殊加成！' : ''}
  ...
  lottery: router({
     ...
     */
    purchaseAdvice: publicProcedure
      .input(z.object({
        targetDate: z.string().optional(),
        weatherElement: z.string().optional(),
      ...
      .query(async ({ input }) => {
        ...
        const moonInfo = getMoonPhase(date);
        const hourEnergy = getCurrentHourEnergy(dateInfo.dayPillar.stem);
        const tenGod = getTenGod(dateInfo.dayPillar.stem);
        const tarotCard = calculateTarotDailyCard(date.getMonth() + 1, date.getDate());
        ...
        const hourBonus = FAVORABLE_ELEMENTS.includes(hourElement as any) ? 0.5 : -0.3;
    ...
    indexHistory: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        // 取得使用者購買記錄
        const scratchLogs = input?.userId ? await getScratchLogs(input.userId) : [];
        ...
        const calcDayScore = (d: Date) => {
          ...
          const hourEnergy = getCurrentHourEnergy(dateInfo.dayPillar.stem);
          const tenGod = getTenGod(dateInfo.dayPillar.stem);
          const tarotCard = calculateTarotDailyCard(d.getMonth() + 1, d.getDate());
          const tenGodLotteryMap: Record<string, number> = {
            偏財: 9, 正財: 7, 食神: 8, 傷官: 6,
            七殺: 4, 正官: 5, 比肩: 3, 劫財: 2,
            偏印: 4, 正印: 5,
    ...
    dailyReport: publicProcedure
      ...
      }> = [];
      {
        ...
        const calcWeekDayScore = (d: Date) => {
          ...
          const fs = tgMap[tg] ?? 5;
          const elMap: Record<string, number> = { '火': 2.0, '土': 1.5, '金': 0.5, '水': -2.0, '木': -1.5 };
        ...
        weeklyLotteryScores.forEach(s => { if (s.compositeScore === maxScore) s.isBest = true; });
      }
