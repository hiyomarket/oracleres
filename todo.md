...
### Bug 修復
- [x] NPC 節點 SQL 錯誤：select region from game_map_nodes where = ? （缺少欄位名稱）
- [x] 前往紫色 NPC 區域時報錯（修復 realm2/tower/abyss 節點連接）

### 戰鬥前後排顯示
- [x] 個人戰鬥中寵物和角色應呈現前後排狀態
- [x] BattleWindow 顯示前排（寵物）和後排（玩家）的視覺區分

### 潛能分配系統重設計
- [x] 改為五行元素分配制（分配五行點數→影響面板屬性）
- [x] 解決 1 等角色就能打 10 等怪的數值失衡問題
- [x] 設計合理的五行→屬性轉換公式

### 伺服器重置功能完善
- [x] 清除玩家道具/背包
- [x] 清除拍賣行資料
- [x] 檢查並清除所有相關遊戲資料（新增 20+ 表清除）

### 裝備欄分類與疊加問題
- [x] 裝備欄按部位分類正確顯示
- [x] 裝備不應疊加（每件獨立存儲）
- [x] 點選指定部位能找到對應裝備（修復 slot 映射）

### 強化系統
- [x] 串接強化卷軸系統
- [x] 修復強化 UI 介面（重寫 EnhancePage.tsx）
- [x] 確保裝備獨立後可正常強化
- [x] 21 個 vitest 測試全部通過（五行潛能、強化引擎、地圖節點、重置引擎）

## v5.10 - 引擎調控 + 道具整理 + 強化完善

### 後台引擎調控
- [x] 新增角色升級經驗值曲線參數到引擎調控面板（V3公式A/B/C可調）
- [x] 對齊屬性平衡參數與現有遊戲制度（五行潛能加成、屬性上限、每級潛能點數）
- [x] 確保所有關鍵參數都可在後台即時調整

### 道具圖鑑整理
- [x] 修正所有非中文顯示的道具名稱（getInventory 優先從 DB 圖鑑讀取名稱）
- [x] 修復消耗品（補血/補魔/復活）只對己方人員生效（combatEngineV2 加入 team 驗證）
- [x] 道具圖鑑新增可疊加/不可疊加選項（已有 stackable + stackLimit）

### 強化系統完善
- [x] 確認強化系統前後端串接正常（修復卷軸 use_effect JSON、新增祝福卷軸）
- [x] 新增強化預覽功能（getFullEnhancePreview API + 完整 +0~+20 路線預覽表）
- [x] 優化強化系統 UI 介面（預覽按鈕、色階表、強化動畫）
- [x] 撰寫強化系統測試（19 個 vitest 測試全部通過）

## v5.11 - 戰鬥平衡面板 + AI圖鑑隨機性 + 角色管理完善

### 戰鬥平衡設計面板
- [x] 補齊所有戰鬥相關欄位（新增 12 項裝備加成欄位 + healPower/hitRate 顯示）
- [x] 讓 AI 生成修改數值時能更完善（模擬器支援裝備加成輸入）

### AI 全圖鑑平衡系統
- [x] 怪物生成增加隨機性變數（±8~15% 浮動 + balanceFix 隨機取值）
- [x] 裝備生成增加隨機性變數（calculateEquipStats 範圍內隨機取值）
- [x] 道具生成增加隨機性變數（applyVariance 全局套用）

### 角色管理面板
- [x] 修復角色管理中數值為 0 的問題（後端新增 calcAgentFullStats 計算）
- [x] 新增裝備欄顯示（GM 可查看玩家裝備）
- [x] 新增道具欄顯示（GM 可查看玩家道具）
- [x] 12 個 vitest 測試全部通過（AI 隨機性、引擎配置、戰鬥屬性、道具目標驗證）

## v5.12 - 技能學習 bug 修復 + 改名

- [x] 修復天命考核（技能學習）無法學習技能的 bug（根因：USK_155 連擊的 learn_cost 使用舊格式 {name,count} 而非 {itemId,qty}）
- [x] 檢查所有 74 個技能的 learn_cost 格式，僅 1 筆不一致，已修正
- [x] 前台「天命考核」改名為「技能學習」（GameTabLayout.tsx）
- [x] 後端新增防禦性格式正規化（同時支援 {itemId,qty} 和舊格式 {name,count}）
...
## v5.14 - 道具清理 + 商店預覽 + 技能學習簡化 + 管理收攏

- [x] 清理背包中無圖鑑的舊格式道具（12 筆孤兒道具已刪除）
- [x] 商店購買前已有道具詳情預覽（ItemDetailModal 已完整實作）
- [x] 技能學習簡化：新增 directLearn 一鍵學習 procedure，只檢查金幣 + 道具，移除任務鏈依賴
- [x] 技能管理收攞：移除「系統管理」>「天命考核」 tab，技能學習設定統一在技能圖鑑管理
- [x] 前台 game/quest-skills 頁面重寫為一鍵學習流程（移除任務鏈步驟顯示）
...
## v5.17 - 戰鬥技能目標選擇 + 商店搜尋修復 + 陽宅開運功能規劃

- [x] 修復戰鬥中技能無法選擇目標（加入己方目標選擇、吸血機制、技能類型判定）
- [x] 修復後台商店新增物品時，搜尋道具/裝備失敗（新增 getAllItems 和 getAllEquipments 後端端點）
- [ ] 研究陽宅開運功能研究報告並規劃首頁呈現方式
- [x] 修復裝備強化後素質未增加的問題（強化成功但屬性沒有實際提升）

## v5.18 - 裝備系統大改造

- [x] 修復裝備實例獨立性：同名裝備不再共用，以 inventory ID 作為裝備識別（核心 bug）
- [x] 修復裝備/卸下邏輯：同名裝備不會同時裝上/卸下
- [x] 修復裝備顯示：裝備名稱顯示強化等級（如「鐵劍+5」而非只有 equipId）
- [x] 修復強化數值同步：強化預覽要與後台強化設定同步顯示
- [x] 修復強化右上角預覽：與強化設定數值同步
- [x] 新增一鍵強化到安定值功能（需檢查玩家強化卷軸數量）
- [x] 新增裝備成功慶祝動畫（提升成就感）

## v5.19 - 強化機制與裝備特效修復

- [x] 修復強化結果仍走舊機制：強化執行的成功率/失敗率/碎裂率要使用後台動態設定，非硬編碼
- [x] 修復+9裝備在裝備欄未顯示對應顏色特效

## v5.20 - 戰鬥系統 bug 修復與 UI 優化

- [x] 修復單人討伐崩潰：BattleWindow 中 Cannot access 'k' before initialization
- [x] 修復組隊戰鬥無法指定我方夥伴放補血技能
- [x] 整合 Boss 系統數值到新系統欄位（移除原始碼輸入，改為後台欄位）
- [x] 戰鬥 UI 優化：敵我雙方顯示頭像/圖片（人物大頭貼、魔物生成圖、寵物圖片）
- [x] Boss 戰敗也要有結算畫面，不直接跳出

## v5.21 - 戰鬥體驗優化

- [x] 修復回合制戰鬥中人物頭像未顯示的問題
- [x] 新增戰鬥結算畫面「傷害統計」頁籤（每位隊員造成傷害/治癒量）
- [x] 優化爆擊/格檔等關鍵事件的視覺提示（更明顯的動畫效果）
- [x] 設計戰鬥背景圖與背景音樂系統
- [x] 修復 Boss 新增表單：對齊魔物圖鑑（名稱/屬性/等級/數值從魔物圖鑑選取填入）
- [x] 修復 Boss 技能列表：技能可從技能圖鑑選擇，不需手動輸入
- [x] 修正 AI 全圖鑑平衡系統：魔物五行比例限制為 5:5/4:6/3:7/2:8/1:9/0:10，主屬性佔 50-100%

## v5.22 - 戰鬥場景系統與修正

- [x] 修復回合制戰鬥角色名稱：顯示正確角色名稱而非「修行者」
- [x] 組隊邏輯帶入角色 ID，確保每位隊員正確識別
- [x] 修正 AI 全圖鑑平衡系統：魔物平衡使用自訂規則（balanceRules）而非舊邏輯
- [x] 實作戰鬥場景背景圖系統（野外/副本/Boss 戰各有專屬背景）
...
## v5.28 - 神喻穿搭引擎 V3.0 智能化升級

### 修改一：五行上限保護機制
- [x] scanElementProtection() 函數：掃描五行比例，35%/40%/45% 三級保護
- [x] sufficient 級（35%）：提示已足夠，不需再補
- [x] overpower 級（40%）：自動替換穿搭顏色，避開過旺五行
- [x] critical 級（45%）：強制切換為剋制色系
- [x] 保護觸發時自動替換上衣/下身/鞋子/配件顏色
- [x] 保護結果附加到穿搭輸出（protectionTriggered 陣列）

### 修改二：流日脈絡分析層
- [x] determineSourceNature() 函數：判定根旺/借旺/虛旺
- [x] 根旺（本命>=20%）：穩定發揮，過旺時建議收斂
- [x] 借旺（本命10-20%）：把握今日，環境助力
- [x] 虛旺（本命<10%）：把握今日但不宜過度
- [x] dailyContextAnalysis 附加到穿搭輸出

### 修改三：推理文案升級
- [x] reasoning 文案整合五行保護狀態和脈絡分析
- [x] 保護觸發時說明「為什麼不穿某色」
- [x] 脈絡分析時說明「今日五行來源」
...
## v5.31 - 陽宅開運強化（化解物品庫 + 照片識別 + 模塊整合）

### 化解物品庫
- [x] 搜尋真實風水化解物品資料（功能/五行/價格/適用場景）
- [x] 建立 seed script 寫入預設化解物品庫（30 筆）
- [x] 執行 seed script 初始化 DB

### 照片上傳 + LLM Vision 形煞識別
- [x] OfficeFengShui.tsx 加入照片上傳 Tab
- [x] 前端：照片選取/拍攝 + base64 上傳至 S3
- [x] 後端：yangzhai.uploadPhoto + analyzeOfficePhoto（LLM vision 分析形煞）
- [x] 前端：顯示 LLM 識別結果（形煞類型 + 嚴重度 + 化解建議）

### 前台模塊整合
- [x] 確認現有模塊管理系統結構（modules 資料表 + 後台管理）
- [x] 陽宅開運加入模塊管理（moduleId: module_fengshui）
- [x] SharedNav 模塊顯示邏輯與陽宅開運連動
- [x] 後台可啟停陽宅開運模塊

### 後台管理整合
- [x] 後台主導航加入「陽宅管理」入口（與其他管理功能同級）
- [x] 確認後台管理頁面有清晰的陽宅管理入口
- [x] 儲存 V5.31 checkpoint

### 老師建議優化（v5.31 補充）
- [x] 問卷加入「門在你座位的哪個方向」問題（這是風水核心判斷依據）
- [x] 移除所有 LLM prompt 和前端文案中的專業術語（八宅明鏡、玄空飛星等），改用白話說法
- [x] 分析邏輯調整：將門的方位納入座位吉凶計算

## v5.32 - 陽宅開運 UX 強化

### 問卷優化
- [x] 門的方位問題改為第1題（最核心的問題先問）
- [x] 門在正後方時顯示即時警示訊息（紅色提示框）
- [x] 結果報告中特別標示「門在正後方」場景的警示

### 社群分享
- [x] 分析結果頁加入分享按鈕（LINE/Facebook/複製連結）
- [x] 分享內容包含環境分數和主要問題摘要

### 後台化解物品 CRUD
- [x] YangzhaiAdmin.tsx 加入化解物品新增 Dialog
- [x] YangzhaiAdmin.tsx 加入化解物品編輯功能（RemedyFormDialog）
- [x] YangzhaiAdmin.tsx 加入化解物品刪除確認（AlertDialog）
- [x] 後端 API deleteRemedyItem 新增（硬刪除）

### 照片上傳動畫
- [x] 照片選取後顯示預覽縮圖
- [x] 上傳中顯示進度動畫（旋轉圖示 + 文字提示）
- [x] LLM 分析中顯示「AI 正在掃描你的辦公環境...」等待動畫
- [x] 儲存 V5.32 checkpoint

## v5.33 - 問卷升級 + 手機羅盤

### 問卷升級（老師建議）
- [x] 第7、8題改為複選題（可同時選多個問題）
- [x] 加入廁所問題：廁所在你辦公室的哪個方向？（無/前方/後方/左方/右方）
- [x] 加入鏡子問題：辦公室有沒有鏡子對著你？（無/正面/側面/背後）
- [x] 加入魚缸問題：辦公室有沒有魚缸？（無/有，在哪個方位）
- [x] 後端 formShaDetector 納入廁所/鏡子/魚缸的分析邏輯（位置對聚財、位置錯破財爛桃花）

### 手機羅盤功能
- [x] 建立 CompassWidget 元件（DeviceOrientationEvent API）
- [x] 手機方位感應：請求權限 → 顯示即時羅盤指針
- [x] 整合進 OfficeFengShui 問卷：座位朝向可直接用羅盤偵測
- [x] 桌機 fallback：無感應器時顯示手動選擇方向的 UI
- [x] iOS 需要 requestPermission()，Android 直接監聽
