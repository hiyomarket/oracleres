...
### 生命靈數計算邏輯修正（最重要）
- [ ] 全站審查：找出所有生命靈數計算相關檔案
- [ ] 修正外層靈數邏輯：日期 ≤22 時不再相加（16=塔，非1+6=7）
- [ ] 修正月份邏輯：月份 >10 才相加（11月=1+1=2，2月保持2）
- [ ] 修正高階靈數顯示：記錄十位數與個位數作為高階能量
- [ ] 驗證 ex1（1984/11/26）：外層8、中層10、靈魂22、主要5
- [ ] 驗證 ex2（1999/2/16）：外層16（塔），不是7（戰車）

### 策略分布白話說明 + 今日vs昨日策略對比
- [ ] 本週策略分布：每個策略加入白話說明與直覺建議
- [ ] 神諭穿搭頁：策略標題旁加入昨日策略名稱與能量轉換說明

### 策略閾值整合 strategyEngine.ts DB 快取
- [ ] strategyEngine.ts 加入 DB 閾值讀取（5分鐘快取）
- [ ] 策略判定邏輯改用 DB 閾值取代硬編碼常數

### 簽到日曆移至個人下拉選單 + Profile 白話化
- [ ] 簽到日曆從 /profile 移至 Header 個人下拉選單
- [ ] Profile 頁文字白話化：專業術語改為一般用戶看得懂的說法
